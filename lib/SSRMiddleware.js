// @semantq/ssr/lib/SSRMiddleware.js
export class SSRMiddleware {
  ...
  async handle(req, res, next) {
    ...
    try {
      ...
      // Fetch data using SemantqQL's service layer
      const service = await this.getService(resolved.resource_type);
      const data = await service.getById(resolved.resource_id);
      ...
      // Render HTML
      const html = await this.renderer.render(resolved.resource_type, data, 'detail');
  ...
  parsePath(path) {
    // Handle /posts/hello-world -> resource: 'posts', identifier: 'hello-world'
    // Handle /post/posts/hello-world -> resource: 'post', identifier: 'hello-world'
  ...
  async getService(resourceType) {
    // Dynamically import the service from SemantqQL
